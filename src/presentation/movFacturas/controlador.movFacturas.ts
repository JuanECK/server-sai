...
export class MovFacturaControlador {

    constructor(
        public readonly movProveedorServicio:MovFacturaServicio,
    ){}
    ...
    cargaMovFacturaId = ( req:Request, res:Response ) =>{

        this.movProveedorServicio.cargaMovFacturaId(req.body.id)
        .then( ( response ) => res.json( response ) )
        .catch( error => this.manejadorErrores( error, res ) )
    ...
    setActualizaMovFactura = ( req:Request, res:Response ) =>{
        
        const [ error, actualizaMovFacturaDto ] = ActualizaMovFacturaDto.crear( req.body.formulario )
        if( error ) return res.status( 400 ).json( { error } )


        this.movProveedorServicio.setActualizaMovFactura( actualizaMovFacturaDto! )
    ...
    agregaMovFactura = ( req:Request, res:Response ) =>{

        const [ error, agregarMovFacturaDto ] = AgregarMovFacturaDto.crear( req.body.formulario )
        if( error ) return res.status( 400 ).json( { error } )

        this.movProveedorServicio.agregaMovFactura( agregarMovFacturaDto! )
        .then( ( resultado ) => { res.json( resultado ) } )
        .catch( error => this.manejadorErrores( error, res ) )
